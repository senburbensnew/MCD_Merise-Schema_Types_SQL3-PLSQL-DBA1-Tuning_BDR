...
begin
	IF objectType NOT IN ('TABLE',  'TABLESPACE') THEN
		raise_application_error(-20000, 'L''objet à analyser n''est pas : un segment, une table et un tablespace');
		raise badObjectType;
	end if;
...
-- 2.1 Créer une grosse table appelée allObjects
...
-- Vérifier à nouveau l'espace occupé.
select sum(bytes)from USER_SEGMENTS 
where segment_name = 'ALLOBJECTS';
/*
SUM(BYTES)
----------
  12582912
*/
-- Malgré la suppression des lignes l'espace
-- reste occupé.

-- 2.2 Analyser la table appelée ALLOBJECTS 
...
-- Appeler la procédure d'anlyse execSegmentAdvisor
begin
execSegmentAdvisor(
...
--- recommandation
--- consulter les actions (diagnostic) recommandées par SA
...
-- Avant d'appliquer vérifier ces recommandations
-- depuis OEM.
...
--Compacte, Déplace les lignes et ajuste le HWM 
--(définit un nouveau HWM). Ajuste aussi les indexes 
--(option cascade)
...
-- Créer une grosse table appelée allObjects
